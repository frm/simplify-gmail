    ...
    switch (key) {
      ...
      case "dateGrouping":
        ...
        if (simplSettings.dateGrouping) {
          ...
          if (threadlistObserver) threadlistObserver.disconnect();
        }
        break;
      case "hideUnreads":
...
function showNotification(msg, actions, hideAfter) {
  ...
  if (actions == "settingsLink") {
    notificationBox.innerHTML +=
      '<br><button id="openSettings">Simplify settings</button>';
    notificationBox.innerHTML +=
...
function detectSplitView() {
  ...
  if (offlineActive && detectSplitViewLoops == 0) {
    ...
    if (splitViewToggle) {
      ...
      const splitViewActionBar = document.querySelectorAll(
        'div[role="main"] > .G-atb'
      );
      if (splitViewActionBar) {
        if (splitViewActionBar.length > 0) {
          ...
          if (simplifyDebug)
            console.log("Split view enabled but set to No Split");
      ...
      detectSplitViewLoops++;
      if (simplifyDebug)
        console.log("Detect preview pane loop #" + detectSplitViewLoops);
...
function detectNumberOfAddOns() {
  ...
  const numberOfAddOns =
    parseInt(
      document.querySelectorAll('.bAw div[role="tablist"] > div[role="tab"]')
        .length
    ) - 2;
  if (numberOfAddOns > 0) {
